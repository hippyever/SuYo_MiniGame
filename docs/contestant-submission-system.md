...
- 游戏封面
- 演示视频
- 作品文件（ZIP、7Z 或 RAR，最大 2GB）
- 负责人姓名
- 队伍名称
- 至少一项制作人员信息

创作手记、成员头像、成员职能、成员工作简述和一句话介绍可选。
...
普通内容在截止后的修改不会触发补交。作品文件替换需要网页内二次确认，并永久保存修改前后的文件记录。补交标记不会因重新上传旧版本而自动消失。管理员可以依据审计撤销当次标记，后续再次触发时系统仍会重新标记。
...
- 队友可以编辑作品内容，只能修改自己的姓名、头像、职能与工作简述。
- 工作简述最多 300 字，用于说明成员在 Demo 中实际完成的模块、功能或内容，并在公开作品详情中随成员资料展示。
...
- 下载项只保留一个“作品文件”，支持 ZIP、7Z、RAR，最大 2GB。
